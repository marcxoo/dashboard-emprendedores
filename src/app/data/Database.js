...
export class Database {
  ...
  async addSurveyResponse(surveyId, responseData) {
    ...
    const { data: liveSurvey, error: liveSurveyError } = await supabase
      .from('custom_surveys')
    ...
    if (liveSurveyError) {
      console.error('Error validating survey status:', liveSurveyError);
      return false;
    }

    const isClosed = liveSurvey?.active === false;
    const liveLimit = liveSurvey?.response_limit;
    const liveCount = liveSurvey?.survey_responses?.length || 0;
    const isFull = liveLimit && liveLimit > 0 && liveCount >= liveLimit;
