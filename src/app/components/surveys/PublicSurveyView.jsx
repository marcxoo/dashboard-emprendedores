...
function PublicSurveyView() {
        ...
        const fetchSurvey = async () => {
            try {
                ...
                if (data) {
                    ...
                    setIsClosed(formattedSurvey.active === false);
    ...
    const handleSubmit = async (e) => {
        ...
        if (survey.active === false || isClosed) {
            setIsClosed(true);
            return;
        }
        ...
        try {
            const { data: latestSurvey, error: latestError } = await supabase
                .from('custom_surveys')
                .select('active, response_limit, survey_responses(id)')
                .eq('id', id)
                .single();

            if (latestError) throw latestError;

            const latestCount = latestSurvey?.survey_responses?.length || 0;
            const latestLimit = latestSurvey?.response_limit;
            const latestClosed = latestSurvey?.active === false;
            const latestFull = latestLimit && latestLimit > 0 && latestCount >= latestLimit;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
                <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center border border-slate-100">
                    ...
                    <h1 className="text-2xl font-bold text-slate-900 mb-2">Encuesta Finalizada</h1>
                    <p className="text-slate-600 mb-6">
                        Este formulario ya fue marcado como finalizado y no acepta más registros.
                    </p>
