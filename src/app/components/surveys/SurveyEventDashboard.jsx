...
function SurveyEventDashboard() {
    ...
    const getSurveyStatus = (survey) => {
        ...
        if (survey.active === false) {
            return {
                ...
                badgeClass: 'bg-slate-500/20 border-slate-500/30 text-slate-300'
            };
        }
    ...
    const handleToggleSurveyClosed = async (survey) => {
        const shouldClose = survey.active !== false;
        const confirmText = shouldClose
            ? '¿Marcar este formulario como finalizado? Ya no aceptará nuevos registros.'
            : '¿Reabrir este formulario para aceptar nuevos registros?';

        if (!window.confirm(confirmText)) return;

        const success = await updateCustomSurvey(survey.id, { active: !shouldClose });
        if (success) {
            showToast(
                shouldClose ? 'Formulario marcado como finalizado' : 'Formulario reabierto',
                'success'
            );
        } else {
            showToast('No se pudo actualizar el estado del formulario', 'error');
        }
    };
            ...
            <main className="flex-1 lg:ml-72 min-h-screen pt-16 lg:pt-0 transition-all duration-300 w-full max-w-full overflow-x-hidden relative z-10">
                ...
                <div className="p-4 lg:p-8 max-w-5xl mx-auto w-full">
                    ...
                    {view === 'list' && viewingResultsId && (() => {
                        ...
                        if (!survey) {
                            return (
                                <div className="bg-slate-800/40 backdrop-blur-xl rounded-[2rem] p-10 border border-white/10 text-center">
                                    <h3 className="text-2xl font-black text-white mb-3">Formulario no disponible</h3>
                                    <p className="text-slate-400 mb-6">No se pudo cargar este formulario. Vuelve a la lista e intenta de nuevo.</p>
                                    <button
                                        onClick={() => setViewingResultsId(null)}
                                        className="px-6 py-3 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-white font-bold transition-colors"
                                    >
                        ...
                        const filteredResponses = responses.filter(r => {
                            ...
                            const matchesSearch =
                                (answers['Nombre Completo']?.toLowerCase().includes(searchLower)) ||
                                (answers['Correo Electrónico']?.toLowerCase().includes(searchLower)) ||
                                (answers['Email']?.toLowerCase().includes(searchLower)) ||
                                (Object.values(answers).some(val => String(val).toLowerCase().includes(searchLower)));

                            if (!matchesSearch) return false;

                            if (filterBy === 'with_email') {
                                return (answers['Correo Electrónico'] || answers['Email'] || answers['Correo'])?.includes('@');
                            }
                                ...
                                <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-900/50 p-4 rounded-3xl border border-white/5">
                                    ...
                                    <div className="flex gap-2 w-full md:w-auto">
                                        ...
                                        <button
                                            onClick={async () => {
                                                try {
                                                    const questions = surveyQuestions.map(q => q.label);
                                                    const headers = ['Fecha', ...questions];
                                                    const rows = survey.responses.map(response => {
                                            ...
                                            <tbody className="divide-y divide-white/5">
                                                {(() => {
                                                    ...
                                                    return filteredResponses.length === 0 ? (
                                                        <tr>
                                                            <td colSpan={surveyQuestions.length + 4} className="p-16 text-center text-slate-500 font-medium">
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="p-5 md:p-6 flex-1 relative flex flex-col">
                                                                        ...
                                                                        <div className="flex justify-between items-start mb-4">
                                                                            ...
                                                                            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${status.badgeClass}`}>
                                                                    ...
                                                                    <div className="px-5 md:px-6 py-4 bg-white/5 border-t border-white/5 grid grid-cols-5 gap-2">
                                                                        <button
                                                                        ...
                                                                        <button
                                                                            onClick={() => handleToggleSurveyClosed(survey)}
                                                                            className={`col-span-1 p-2 rounded-xl flex items-center justify-center transition-all duration-300 ${survey.active === false
                                                                                ? 'text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10'
                                                                                : 'text-amber-400 hover:text-amber-300 hover:bg-amber-500/10'
                                                                                }`}
                                                                            title={survey.active === false ? 'Reabrir formulario' : 'Marcar como finalizada'}
                                                                        >
                                                                            {survey.active === false ? <RefreshCw size={20} strokeWidth={1.5} /> : <CheckSquare size={20} strokeWidth={1.5} />}
                                                                        </button>
                                                                        <button
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <div className="flex justify-between items-start mb-2">
                                                                            ...
                                                                            <span className={`w-2 h-2 rounded-full shrink-0 mt-1 ${status.key === 'closed'
                                                                                ? 'bg-slate-400 shadow-[0_0_5px_rgba(148,163,184,0.4)]'
                                                                                : status.key === 'full'
                                                                        ...
                                                                        <div className="flex items-center justify-between mt-3">
                                                                            ...
                                                                            <div className="flex items-center gap-1 opacity-60 group-hover/item:opacity-100 transition-opacity">
                                                                                ...
                                                                                <button
                                                                                    onClick={() => handleToggleSurveyClosed(survey)}
                                                                                    className={`p-1.5 rounded-lg transition-colors ${survey.active === false
                                                                                        ? 'text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10'
                                                                                        : 'text-amber-400 hover:text-amber-300 hover:bg-amber-500/10'
                                                                                        }`}
                                                                                    title={survey.active === false ? 'Reabrir formulario' : 'Marcar como finalizada'}
                                                                                >
                                                                                    {survey.active === false ? <RefreshCw size={14} /> : <CheckSquare size={14} />}
                                                                                </button>
                                                ...
                                                >
                                                    ...
                                                    <div className="p-5 md:p-8 pb-6 flex-1 relative flex flex-col">
                                                        ...
                                                        <div className="flex justify-between items-start mb-6">
                                                            ...
                                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 text-[10px] font-black rounded-lg uppercase tracking-wider border ${status.badgeClass}`}>
                                                        ...
                                                        <div className="bg-slate-900/50 rounded-2xl p-5 border border-white/10 group-hover:border-white/20 transition-colors mt-auto">
                                                            ...
                                                            {hasLimit ? (
                                                                <div className="h-2 bg-slate-700/50 rounded-full overflow-hidden">
                                                                    <div
                                                                        className={`h-full rounded-full transition-all duration-1000 ease-out shadow-lg relative ${status.key === 'full' ? 'bg-red-500' : status.key === 'closed' ? 'bg-slate-500' : 'bg-indigo-500'}`}
                                                    ...
                                                    {/* Actions */}
                                                    <div className="p-2 grid grid-cols-5 gap-1 bg-black/20 border-t border-white/5">
                                                        <button
                                                            onClick={() => copyToClipboard(survey.id)}
                                                        ...
                                                        <button
                                                            onClick={() => handleToggleSurveyClosed(survey)}
                                                            className={`col-span-1 py-4 rounded-2xl flex items-center justify-center transition-all duration-300 ${survey.active === false
                                                                ? 'text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10'
                                                                : 'text-amber-400 hover:text-amber-300 hover:bg-amber-500/10'
                                                                }`}
                                                            title={survey.active === false ? 'Reabrir formulario' : 'Marcar como finalizada'}
                                                        >
                                                            {survey.active === false ? <RefreshCw size={18} /> : <CheckSquare size={18} />}
                                                        </button>
                                                        <button
                                                            onClick={() => handleDelete(survey.id)}
